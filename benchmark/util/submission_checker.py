...
class Config():
  ...
  def set_type(self, submission_type):
    if submission_type is None and self.version in ["v1.0", "v1.1"]:
...
def check_results_dir(config,
  ...
  head = [
      ...
      "Model", "MlperfModel", "Result", "ResultUnit", "Accuracy",
      "HostProcessorModelName",
  ...
  def log_result(submitter,
    ...
    csv.write(
        fmt.format(submitter,
                   ...
                   unit,
                   acc,
                   system_json.get("Processor(s) Name"),
                   power_metric > 0,
                   power_metric,
                   power_unit))
