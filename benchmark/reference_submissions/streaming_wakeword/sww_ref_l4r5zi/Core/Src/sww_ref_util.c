...
void delay_us(int delay_len_us)
{
	...
	int delay_start = TH_TIMER16_GET();
	while(TH_TIMER16_GET() < delay_start + 1 );
}
...
//	acquire_and_process_data(in_data);
	...
	timer_start = TH_TIMER16_GET();
	/*  Call inference engine */
	aiRun(in_data, out_data);
	timer_stop = TH_TIMER16_GET();
...
void stop_detection(char *cmd_args[])
{
	...
	case Streaming:
		...
		g_i2s_status = th_dma_stop(&hsai_BlockA1);
		g_i2s_state = Idle;
...
void start_detection(char *cmd_args[])
{
	...
	else
	{
		...
		ee_set_processing_pin_high();  // end of processing, used for duty cycle measurement
		delay_us(1);
		ee_set_processing_pin_low();  // end of processing, used for duty cycle measurement


		g_i2s_status = th_dma_receive(&hsai_BlockA1, (uint8_t *)g_i2s_current_buff, g_i2s_chunk_size_bytes/2);
...
void ee_process_command(char *full_command)
{
	...
	else if (strcmp(cmd_args[0], "proc_hi") == 0) // These next two are mostly useful for testing
		ee_set_processing_pin_high();
	else if (strcmp(cmd_args[0], "proc_lo") == 0)
		ee_set_processing_pin_low();
	else if (strcmp(cmd_args[0], "infer_wav") == 0)
		ee_infer_static_wav(cmd_args);
...
void process_chunk_and_cont_capture(void *hsai)
{
	...
	if (g_int16s_read + g_i2s_chunk_size_bytes/2 <= g_i2s_wav_len) // there is space left for a full chunk
		g_i2s_status = th_dma_receive((SAI_HandleTypeDef *)hsai, (uint8_t *)g_i2s_current_buff, g_i2s_chunk_size_bytes/2);
	else
		reading_complete = 1;
